...
│   ├── rolling-updates.md          # Rolling update strategy, rollback, hands-on
│   ├── deployment-strategies.md    # Rolling Update vs Recreate vs Blue-Green vs Canary
│   ├── production-cicd.md          # Full CI/CD pipeline: test → build → push → deploy
│   ├── monitoring-logging.md       # Metrics vs logs vs events, health checks, kubectl commands
│   └── interview-scenarios.md      # "Deployment succeeded but users see old version" walkthrough
│
├── .github/
│   └── workflows/
...
kubectl logs -f deploy/nodejs-deployment   # follow logs from any pod in the deployment

# Events (cluster-wide + per-resource)
kubectl get events
kubectl get events --sort-by=.metadata.creationTimestamp   # chronological, easiest to read
kubectl get events -w                                        # watch live
kubectl get events --field-selector involvedObject.name=<pod-name>

# Resource usage (requires Metrics Server — see Monitoring section below)
kubectl top pods
kubectl top nodes
...
Three separate questions, three separate tools — see `docs/monitoring-logging.md`
for the full writeup.

| Question | Source | Command |
|---|---|---|
| Is it healthy right now? | Health checks (continuous) | liveness/readiness probes in `deployment.yaml` |
| How much load is it under? | Metrics | `kubectl top pods`, Metrics Server |
| What exactly went wrong? | Logs | `kubectl logs`, `kubectl logs -f` |
| What did the control plane do? | Events | `kubectl get events`, `kubectl describe pod` |

**Debugging order that saves time:** check `kubectl describe pod` (Events
section) *before* `kubectl logs`. A Pod that fails to schedule or pull its
image never gets the chance to write an application log — Events are the
only record of what went wrong in that case.

```bash
# The four-command triage sequence for "something's wrong with a Pod"
kubectl get pods                                    # is it even running?
kubectl describe pod <pod-name>                      # what did the control plane do?
kubectl logs <pod-name>                              # what did the app itself log?
kubectl top pods                                     # is it resource-starved?
```

**Health check config** (already in `kubernetes/deployment.yaml`):
...
**Installing Metrics Server** (required for `kubectl top`, not built in by default):

```bash
kubectl apply -f https://github.com/kubernetes-sigs/metrics-server/releases/latest/download/components.yaml

# Docker Desktop / minikube often need this patch for self-signed kubelet certs:
...
### Rollout stuck: `deployment exceeded its progress deadline`

**Symptom:** `kubectl rollout status` hangs, then fails with
`0 of 3 updated replicas are available`.

**Cause:** New Pods aren't becoming ready — could be a bad image, a failing
readiness probe, or (as hit during this project on EKS) zero worker nodes
available to schedule Pods onto at all.

**Fix:**
```bash
kubectl get pods                        # check for Pending / CrashLoopBackOff
kubectl describe pod <new-pod-name>      # Events section shows the real reason

# If Events shows "no nodes available to schedule pods":
kubectl get nodes                        # confirm zero or NotReady nodes
kubectl config current-context           # confirm which cluster you're on
# → on EKS: scale up or create a node group with eksctl
# → on Docker Desktop: switch context back with kubectl config use-context docker-desktop
```

---

### Deployment completed, but users still see the old version

**Symptom:** `kubectl rollout status` reports success, but the live app
still behaves like the previous version.

**Cause:** One of three layers is stale — the Pod itself, the Service/Ingress
routing, or client-side caching. See `docs/interview-scenarios.md` for the
full investigation flow.

**Fast diagnostic — bypass every layer and ask a Pod directly:**
```bash
kubectl exec -it <pod-name> -- wget -qO- http://localhost:3000/info
```
If this shows the new version, the Pod layer is correct — the bug is in
Service/Ingress routing or client caching. If it shows the old version, the
rollout never actually completed at the Pod level — check
`kubectl get pods -o jsonpath` for the image every Pod is actually running,
and check for a leftover ReplicaSet still scaled above zero with
